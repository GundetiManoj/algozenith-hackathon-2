...
def process_files(folder_path):
    ...
    with open('original codeforces/tf-idf-codeforces/vocab.txt', 'w', encoding='utf-8', errors='replace') as f:
        for key in vocab.keys():
            f.write("%s\n" % key)

    # save the idf values in a text file
    with open('original codeforces/tf-idf-codeforces/idf-values.txt', 'w', encoding='utf-8', errors='replace') as f:
        for key in vocab.keys():
            f.write("%s\n" % vocab[key])

    # save the documents in a text file
    with open('original codeforces/tf-idf-codeforces/documents.txt', 'w', encoding='utf-8', errors='replace') as f:
        for document in documents:
            f.write("%s\n" % ' '.join(document))

    # save the inverted index in a text file
    with open('original codeforces/tf-idf-codeforces/inverted-index.txt', 'w', encoding='utf-8', errors='replace') as f:
        for key in inverted_index.keys():
            f.write("%s\n" % key)
            f.write("%s\n" % ' '.join([str(doc_id) for doc_id in inverted_index[key]]))

folder_path = 'original codeforces\Qdata'
